...
var starCreateBufferId;
   ...
   for (var i = 0; i <= 360; i+=1){
      var j = i * Math.PI / 180;
      var vert1 = [ Math.sin(j) * 0.03, Math.cos(j) * 0.03];
      var vert2 = [0, 0];
...
window.onload = function init()
{
   ...
   starCreateBufferId = gl.createBuffer();
    ...
    canvas.addEventListener("mousedown", function(event){
      ...
      var t = vec2(2 * event.clientX/canvas.width - 1,
         ...
         arrayVertex = t;
         arrayVertex.push(t[0]-0.05);
         arrayVertex.push(t[1]-0.05);
         arrayVertex.push(t[0]+0.05);
         arrayVertex.push(t[1]-0.05);
         arrayVertex.push(t[0]-0.05);
         arrayVertex.push(t[1]-0.025);
         arrayVertex.push(t[0]);
         arrayVertex.push(t[1]-0.075);
         arrayVertex.push(t[0]+0.05);
         arrayVertex.push(t[1]-0.025);
...
function render(){
   ...
   /*-----------------------------------------------------------------------*/
    ...
    gl.bindBuffer( gl.ARRAY_BUFFER, starCreateBufferId );
    gl.bufferData( gl.ARRAY_BUFFER, flatten(8*maxNumvertices), gl.STATIC_DRAW );
